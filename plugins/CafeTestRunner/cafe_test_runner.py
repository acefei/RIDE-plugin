...
from robotide.widgets import Label, ImageProvider, HtmlDialog
...
class CafeTestRunner(Plugin):
    """A plugin for running CAFE tests from within RIDE,
    it provided a way to set individual arguments configuration for the different projects."""
    ...
    valid_profile_name_regex = re.compile("^[A-Za-z][\w ]{1,50}$")
    title = "Cafe Test Runner"

    def __init__(self, application=None):
        Plugin.__init__(self, application, initially_enabled=True,
                        default_settings=self.defaults, name=self.title)
        self.version = "2.0"
        ...
        self._new_profile_text = 'Add New Profile'
        self._plugins_settings = application.settings['Plugins']
    ...
    def enable(self):
        self._test_runner_plugin_toggle(False)
    ...
    def _test_runner_plugin_toggle(self, flag):
        if self._plugins_settings.has_setting('Test Runner'):
            self._plugins_settings['Test Runner'].set('_enabled', flag)
    ...
    def _read_run_profiles(self):
        ...
        self._read_custom_profiles_from_config()
    ...
    def _read_custom_profiles_from_config(self):
        for profile_name in self.gain_settings('custom_profiles'):
            profile = runprofiles.CustomScriptProfile(plugin=self)
            profile.name = profile_name
            self._test_runner.add_profile(profile.name, profile)
    ...
    def gain_settings(self, name, default=None):
        return self._Plugin__settings.get(name, default)
    ...
    def disable(self):
        ...
        self._test_runner_plugin_toggle(True)

    def OnRemoveToolClick(self, evt):
        self._del_custom_profile()

        self.save_setting('profile', 'custom script')
        self.SetProfile(self.profile)
    ...
    def _ask_user_to_save_new_profile(self):
        dlg = wx.TextEntryDialog(self.panel, 'Please enter a name for new profile:', 'Add New Profile', '')
        if dlg.ShowModal() == wx.ID_OK:
            new_profile_name = dlg.GetValue().strip()
            if not self.custom_profile_validation(new_profile_name, self.valid_profile_name_regex):
                return 'custom script'

            self._add_custom_profile(new_profile_name)
    ...
    def _add_custom_profile(self, new_profile):
        custom_profiles = self.gain_settings('custom_profiles')

        # add new profile in settings.cfg
        custom_profiles.append(new_profile)
        custom_profiles.sort()
        self.save_setting('custom_profiles', custom_profiles)

        # add new profile in choice panel, and sort the choices.
        fixed_choices = self.choice.GetItems()[-4:]
        sorted_choices = custom_profiles + fixed_choices
        self.choice.SetItems(sorted_choices)

        # add new profile as TestRunner instance
        profile = runprofiles.CustomScriptProfile(plugin=self)
        profile.name = new_profile
        self._test_runner.add_profile(profile.name, profile)

    def _del_custom_profile(self):
        custom_profiles = self.gain_settings('custom_profiles')
        new_custom_profiles = [v for v in custom_profiles if v != self.profile]
        self.save_setting('custom_profiles', new_custom_profiles)

        # remove selected custom profile in choice panel
        self.choice.Delete(self.choice.GetCurrentSelection())

        # remove selected custom profile from TestRunner
        del self._test_runner.profiles[self.profile]

        # remove selected custom profile in settings.cfg
        for k, _ in self._Plugin__settings.iteritems():
            if self.profile.strip().replace(' ', '_') in k:
                del self._Plugin__settings._config_obj[k]

    def custom_profile_validation(self, new_profile, regex):
        m = regex.match(new_profile)
        if new_profile == '' or not m:
            self.WarningDialog('''<h1>Warning</h1>
            <br>The Profile naming convention follow there basic rules:<br>
            <ul>
                <li>Begin with a letter</li>
                <li>Length must be 1 to 50 characters</li>
                <li>Only contain underline, space and alpha-numeric characters</li>
            </ul>''')
            return False
        elif new_profile in self.gain_settings('custom_profiles'):
            self.WarningDialog('''<h1><br>Warning</h1>
            <p>The profile name [{}] is already in use.<br>
    ...
    def WarningDialog(self, massages):
    ...
    def OnProfileSelection(self, event):
        ...
        if event.GetString() == self._new_profile_text:
            profile_name = self._ask_user_to_save_new_profile()

        self.save_setting("profile", profile_name)
        self.SetProfile(self.profile)
    ...
    def _build_local_toolbar(self):
        ...
        choices.append(self._new_profile_text)
    ...
    def SetProfile(self, profile):
        ...
        # Add a remove button to delete custom profile
        if profile in self.gain_settings('custom_profiles'):
            if not self.local_toolbar.FindById(ID_REMOVE_PROFILE):
                self.local_toolbar.AddLabelTool(ID_REMOVE_PROFILE, "Remove", ImageProvider().TOOLBAR_STOP,
                                                shortHelp="Remove selected profile", longHelp="Remove selected profile")

                self.local_toolbar.Bind(wx.EVT_TOOL, self.OnRemoveToolClick, id=ID_REMOVE_PROFILE)

        # Hide remove button, when selected profile in runprofiles class defined
        else:
            if self.local_toolbar.FindById(ID_REMOVE_PROFILE):
                self.local_toolbar.DeleteTool(ID_REMOVE_PROFILE)
                self.local_toolbar.Unbind(wx.EVT_TOOL, id=ID_REMOVE_PROFILE)
